...
mind_back = on_command(
    "我的背包", aliases={"我的物品"}, priority=10, permission=GROUP, block=True
)
...
set_auction = on_command(
    "群交流会",
    priority=4,
    permission=GROUP and (SUPERUSER | GROUP_ADMIN | GROUP_OWNER),
    block=True,
)
creat_auction = on_fullmatch(
    "举行交友会", priority=5, permission=GROUP and SUPERUSER, block=True
)
offer_auction = on_command("交友", priority=5, permission=GROUP, block=True)
back_help = on_command(
    "背包帮助", aliases={"坊市帮助"}, priority=8, permission=GROUP, block=True
)
            ...
            try:
                if XiuConfig().img:
                    ...
                    await bot.send_group_msg(
                        group_id=int(group_id), message=MessageSegment.image(pic)
                    )
                else:
...
@shop_added_by_admin.handle(
    parameterless=[Cooldown(1.4, isolate_level=CooldownIsolateLevel.GROUP, parallel=1)]
)
async def shop_added_by_admin_(
...
@shop_added.handle(
    parameterless=[Cooldown(1.4, isolate_level=CooldownIsolateLevel.GROUP)]
)
...
async def shop_off_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if shop_data[group_id][str(arg)]["user_id"] == user_id:
        sql_message.send_back(
            user_id,
            shop_data[group_id][str(arg)]["goods_id"],
            shop_data[group_id][str(arg)]["goods_name"],
            shop_data[group_id][str(arg)]["goods_type"],
        ...
        if shop_data[group_id][str(arg)]["user_id"] == 0:  # 这么写为了防止bot.send发送失败，不结算
            ...
            sql_message.send_back(
                shop_data[group_id][str(arg)]["user_id"],
                shop_data[group_id][str(arg)]["goods_id"],
                shop_data[group_id][str(arg)]["goods_name"],
                shop_data[group_id][str(arg)]["goods_type"],
                shop_data[group_id][str(arg)]["number"],
...
@offer_auction.handle(
    parameterless=[Cooldown(1.4, isolate_level=CooldownIsolateLevel.GLOBAL)]
)
async def offer_auction_(
    bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()
):
    group_id = str(event.group_id)
    bot = await assign_bot_group(group_id=group_id)
...
@shop_off_all.handle(
    parameterless=[Cooldown(60, isolate_level=CooldownIsolateLevel.GROUP, parallel=1)]
)
async def shop_off_all_(bot: Bot, event: GroupMessageEvent):
    ...
    for x in range(num):
        ...
        if shop_data[group_id][str(x)]["user_id"] == 0:  # 这么写为了防止bot.send发送失败，不结算
            ...
            sql_message.send_back(
                shop_data[group_id][str(x)]["user_id"],
                shop_data[group_id][str(x)]["goods_id"],
                shop_data[group_id][str(x)]["goods_name"],
                shop_data[group_id][str(x)]["goods_type"],
                shop_data[group_id][str(x)]["number"],
            )
    ...
    list_msg.append(
        {
            "type": "node",
            "data": {"name": f"执行清空坊市ing", "uin": bot.self_id, "content": msg},
        }
    )
    try:
