...
def get_use_equipment_sql(user_id, goods_id):
    ...
    if item_info["item_type"] == "法器":
        ...
        sql_str.append(
            f"UPDATE back set update_time='{now_time}',action_time='{now_time}',state=1 WHERE user_id={user_id} and goods_id={goods_id}"
        )  # 装备
        if in_use_id != 0:
            sql_str.append(
...
def get_user_back_msg(user_id):
    ...
    for user_back in user_backs:
        if user_back.goods_type == "装备":
            l_equipment_msg = get_equipment_msg(
                l_equipment_msg, user_id, user_back.goods_id, user_back.goods_num
            )

        elif user_back.goods_type == "技能":
            l_skill_msg = get_skill_msg(
                l_skill_msg, user_id, user_back.goods_id, user_back.goods_num
            )

        elif user_back.goods_type == "丹药":
            l_elixir_msg = get_elixir_msg(
                l_elixir_msg, user_back.goods_id, user_back.goods_num
            )

        elif user_back.goods_type == "聚灵旗":
            l_xiulianitem_msg = get_jlq_msg(
                l_xiulianitem_msg, user_id, user_back.goods_id, user_back.goods_num
            )

        elif user_back.goods_type == "炼丹炉":
            l_ldl_msg = get_ldl_msg(
                l_ldl_msg, user_id, user_back.goods_id, user_back.goods_num
            )

        elif user_back.goods_type == "药材":
            l_yaocai_msg = get_yaocai_msg(
                l_yaocai_msg, user_id, user_back.goods_id, user_back.goods_num
            )
    if l_equipment_msg:
        l_msg.append("☆------我的装备------☆")
    ...
    if l_yaocai_msg:
        l_msg.append("☆------我的药材------☆")
        l_yaocai_msg.sort(key=takeSecond)
        msg = ""
        for x in l_yaocai_msg:
            msg += x
        l_msg.append(msg)
    return l_msg
...
def get_yaocai_msg(l_msg, user_id, goods_id, goods_num):
    ...
    """
    item_info = items.get_data_by_item_id(goods_id)
    msg = f"{item_info['level']} {item_info['name']} 数量:{goods_num}\n"
...
def check_use_elixir(user_id, goods_id, num):
    ...
    if goods_info["buff_type"] == "level_up_rate":  # 增加突破概率的丹药
        if goods_rank < user_rank:  # 最低使用限制
            ...
            sql_message.update_levelrate(
                user_id, user_info.level_up_rate + goods_info["buff"] * num
            )
            msg = (
                f"道友成功使用丹药：{goods_name}{num}颗，下一次突破的成功概率提高{goods_info['buff'] * num}%!"
            )
        ...
        if goods_rank != user_rank:  # 使用限制
            ...
            if goods_all_num >= goods_info["all_num"]:
                ...
                sql_message.update_levelrate(
                    user_id, user_info.level_up_rate + goods_info["buff"]
                )
        ...
        if user_info.root == "器师":
            ...
            if user_info.hp == user_max_hp and user_info.mp == user_max_mp:
                ...
                msg = (
                    f"道友成功使用丹药：{goods_name}{num}颗，经过境界转化状态恢复了{int(buff * 100 * num)}%!"
                )
                sql_message.update_back_j(user_id, goods_id, num=num, use_key=1)
        ...
        if goods_rank < user_rank:  # 使用限制
            ...
            sql_message.update_user_attribute(
                user_id, user_hp, user_mp, user_atk
            )  # 这种事情要放在update_exp方法里
            sql_message.update_back_j(user_id, goods_id, num=num, use_key=1)
