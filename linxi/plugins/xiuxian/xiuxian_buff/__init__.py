...
out_closing = on_command(
    "出关", aliases={"灵石出关"}, priority=5, permission=GROUP, block=True
)
in_closing = on_fullmatch("闭关", priority=5, permission=GROUP, block=True)
stone_exp = on_command(
    "灵石修仙", aliases={"灵石修炼"}, priority=5, permission=GROUP, block=True
)
...
buff_help = on_command(
    "功法帮助", aliases={"灵田帮助"}, priority=5, permission=GROUP, block=True
)
...
async def ling_tian_up_(bot: Bot, event: GroupMessageEvent):
    ...
    if int(user_info.blessed_spot_flag) == 0:
        ...
        await ling_tian_up.finish()
    LINGTIANCONFIG = {
        "1": {"level_up_cost": 3500000},
        "2": {"level_up_cost": 5000000},
        "3": {"level_up_cost": 7000000},
        "4": {"level_up_cost": 10000000},
        "5": {"level_up_cost": 15000000},
    }
    mix_elixir_info = get_player_info(user_id, "mix_elixir_info")
...
async def blessed_spot_rename_(
...
async def two_exp_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if user_2:
        ...
        if exp_2 > exp_1:
            ...
            impart_two_exp_1 = (
                impart_data_1.impart_two_exp if impart_data_1 is not None else 0
            )
            impart_two_exp_2 = (
                impart_data_2.impart_two_exp if impart_data_2 is not None else 0
            )
            ...
            if limt_2 >= two_exp_limit + impart_two_exp_2:
                ...
                await two_exp.finish()
            max_exp_1 = (
                int(OtherSet().set_closing_type(user_1.level))
                * XiuConfig().closing_exp_upper_limit
            )  # 获取下个境界需要的修为 * 1.5为闭关上限
            max_exp_2 = (
                int(OtherSet().set_closing_type(user_2.level))
                * XiuConfig().closing_exp_upper_limit
            )
...
async def out_closing_(bot: Bot, event: GroupMessageEvent):
    ...
    if not is_type:
        ...
        mainbuffratebuff = (
            mainbuffdata["ratebuff"] if mainbuffdata != None else 0
        )  # 功法修炼倍率
        exp = int(
        ...
        if exp >= user_get_exp_max:
            ...
            result_msg, result_hp_mp = OtherSet().send_hp_mp(
                user_id, int(exp * hp_speed), int(exp * mp_speed)
            )
            sql_message.update_user_attribute(
                user_id, result_hp_mp[0], result_hp_mp[1], int(result_hp_mp[2] / 10)
            )
            msg = "闭关结束，本次闭关到达上限，共增加修为：{}{}{}".format(
                user_get_exp_max, result_msg[0], result_msg[1]
            )
            if XiuConfig().img:
                pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
                await bot.send_group_msg(
            ...
            if str(event.message) == "灵石出关":
                ...
                if exp <= user_stone:
                    ...
                    msg = "闭关结束，共闭关{}分钟，本次闭关增加修为：{}，消耗灵石{}枚{}{}".format(
                        exp_time, exp, int(exp / 2), result_msg[0], result_msg[1]
                    )
                    if XiuConfig().img:
                        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
                        await bot.send_group_msg(
                    ...
                    msg = "闭关结束，共闭关{}分钟，本次闭关增加修为：{}，消耗灵石{}枚{}{}".format(
                        exp_time, exp, user_stone, result_msg[0], result_msg[1]
                    )
                ...
                msg = "闭关结束，共闭关{}分钟，本次闭关增加修为：{}{}{}".format(
                    exp_time, exp, result_msg[0], result_msg[1]
                )
                if XiuConfig().img:
                    pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
                    await bot.send_group_msg(
...
async def mind_state_(bot: Bot, event: GroupMessageEvent):
    ...
    main_buff_rate_buff = (
        main_buff_data["ratebuff"] if main_buff_data is not None else 0
    )
