...
def Player_fight(player1: dict, player2: dict, type_in, bot_id):
    ...
    user1_hp_buff = (
        user1_main_buff_data["hpbuff"] if user1_main_buff_data is not None else 0
    )
    user1_mp_buff = (
        user1_main_buff_data["mpbuff"] if user1_main_buff_data is not None else 0
    )
    try:
    ...
    user_1_impart_hp = (
        user_1_impart_data.impart_hp_per if user_1_impart_data is not None else 0
    )
    user_1_impart_mp = (
        user_1_impart_data.impart_mp_per if user_1_impart_data is not None else 0
    )
    ...
    user2_hp_buff = (
        user2_main_buff_data["hpbuff"] if user2_main_buff_data is not None else 0
    )
    user2_mp_buff = (
        user2_main_buff_data["mpbuff"] if user2_main_buff_data is not None else 0
    )
    try:
    ...
    user_2_impart_hp = (
        user_1_impart_data.impart_hp_per if user_2_impart_data is not None else 0
    )
    user_2_impart_mp = (
        user_1_impart_data.impart_mp_per if user_2_impart_data is not None else 0
    )
    ...
    while True:
        ...
        if player1_skil_open:  # 是否开启技能
            if user1_turn_skip:  # 无需跳过回合
                play_list.append(
                    get_msg_dict(
                        player1, player1_init_hp, f"☆------{player1['道号']}的回合------☆"
                    )
                )
                (
                    user1_hp_cost,
                ...
                if player1_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(
                        ...
                        (
                            ...
                            user1_skill_sh,
                            player1_turn_cost,
                        ) = get_skill_sh_data(player1, user1_skill_date)
                        if user1_skill_type == 1:  # 直接伤害类技能
                            play_list.append(
                                get_msg_dict(player1, player1_init_hp, skill_msg)
                            )
                            player1 = calculate_skill_cost(
                                player1, user1_hp_cost, user1_mp_cost
                            )
                            player2["气血"] = player2["气血"] - int(
                                user1_skill_sh * player2_js
                            )  # 玩家1的伤害 * 玩家2的减伤
                            play_list.append(
                                get_msg_dict(
                                    player1,
                            ...
                            if user1_buff_type == 1:  # 攻击类buff
                                isCrit, player1_sh = get_turnatk(
                                    player1, user1_skill_sh
                                )  # 判定是否暴击
                                if isCrit:
                                    msg1 = "{}发起会心一击，造成了{}伤害\n"
                                else:
                                ...
                                play_list.append(
                                    get_msg_dict(
                                        player1,
                                        player1_init_hp,
                                        msg1.format(player1["道号"], player1_sh),
                                    )
                                )
                                player2["气血"] = player2["气血"] - int(
                                    player1_sh * player2_js
                                )  # 玩家1的伤害 * 玩家2的减伤
                                play_list.append(
                                    get_msg_dict(
                                        player1,
                                ...
                                player1_js = (
                                    player1_f_js - user1_skill_sh
                                    if player1_f_js - user1_skill_sh > 0.1
                                    else 0.1
                                )
                    ...
                    if user1_skill_type == 2:  # 持续性伤害技能
                        ...
                        skill_msg = get_persistent_skill_msg(
                            player1["道号"],
                            user1_skill_date["name"],
                            user1_skill_sh,
                            player1_turn_cost,
                        )
                        play_list.append(
                        ...
                        player2["气血"] = player2["气血"] - int(
                            (user1_skill_sh + player1_sh) * player2_js
                        )
                        play_list.append(
                            get_msg_dict(
                                player1,
                        ...
                        if user1_buff_type == 1:  # 攻击类buff
                            ...
                            player1_js = (
                                player1_f_js - user1_skill_sh
                                if player2_f_js - user1_skill_sh > 0.1
                                else 0.1
                            )
                ...
                play_list.append(
                    get_msg_dict(
                        player1, player1_init_hp, f"☆------{player1['道号']}动弹不得！------☆"
                    )
                )
            ...
            if user1_turn_skip:
                ...
                play_list.append(
                    get_msg_dict(
                        player1, player1_init_hp, msg1.format(player1["道号"], player1_sh)
                    )
                )
                player2["气血"] = player2["气血"] - player1_sh
                play_list.append(
                    get_msg_dict(
                        player1, player1_init_hp, f"{player2['道号']}剩余血量{player2['气血']}"
                    )
                )
        ...
        if player2["气血"] <= 0:  # 玩家2气血小于0，结算
            ...
            if isSql:
                ...
                XiuxianDateManage().update_user_hp_mp(
                    player2["user_id"], 1, int(player2["真元"] / (1 + user2_mp_buff))
                )
            break
        ...
        if player2_skil_open:  # 有技能
            if user2_turn_skip:  # 玩家2无需跳过回合
                play_list.append(
                    get_msg_dict(
                        player2, player2_init_hp, f"☆------{player2['道号']}的回合------☆"
                    )
                )
                (
                    user2_hp_cost,
                    ...
                    user2_skill_type,
                    skill_rate,
                ) = get_skill_hp_mp_data(player2, user2_skill_date)
                if player2_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(
                        ...
                        (
                            ...
                            user2_skill_sh,
                            player2_turn_cost,
                        ) = get_skill_sh_data(player2, user2_skill_date)
                        if user2_skill_type == 1:  # 直接伤害类技能
                            play_list.append(
                                get_msg_dict(player2, player2_init_hp, skill_msg)
                            )
                            player1["气血"] = player1["气血"] - int(
                                user2_skill_sh * player1_js
                            )  # 玩家2的伤害 * 玩家1的减伤
                            play_list.append(
                                get_msg_dict(
                                    player2,
                            ...
                            player2 = calculate_skill_cost(
                                player2, user2_hp_cost, user2_mp_cost
                            )
                            ...
                            if user2_buff_type == 1:  # 攻击类buff
                                isCrit, player2_sh = get_turnatk(
                                    player2, user2_skill_sh
                                )  # 判定是否暴击
                                if isCrit:
                                    msg2 = "{}发起会心一击，造成了{}伤害\n"
                                else:
                                ...
                                play_list.append(
                                    get_msg_dict(
                                        player2,
                                        player2_init_hp,
                                        msg2.format(player2["道号"], player2_sh),
                                    )
                                )
                                player1["气血"] = player1["气血"] - int(
                                    player2_sh * player1_js
                                )
                                play_list.append(
                                    get_msg_dict(
                                        player2,
                                ...
                                player2_js = (
                                    player2_f_js - user2_skill_sh
                                    if player2_f_js - user2_skill_sh > 0.1
                                    else 0.1
                                )
                                player2 = calculate_skill_cost(
                                    player2, user2_hp_cost, user2_mp_cost
                    ...
                    if user2_skill_type == 2:  # 持续性伤害技能
                        ...
                        player1["气血"] = player1["气血"] - int(
                            (user2_skill_sh + player2_sh) * player1_js
                        )
                        play_list.append(
                            get_msg_dict(
                                player2,
                ...
                play_list.append(
                    get_msg_dict(
                        player2, player2_init_hp, f"☆------{player2['道号']}动弹不得！------☆"
                    )
                )
            ...
            if user2_turn_skip:
                ...
                play_list.append(
                    get_msg_dict(
                        player2, player2_init_hp, msg2.format(player2["道号"], player2_sh)
                    )
                )
                player1["气血"] = player1["气血"] - player2_sh
                play_list.append(
                    get_msg_dict(
                        player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"
                    )
                )
        ...
        if player1["气血"] <= 0:  # 玩家1气血小于0，结算
            ...
            if isSql:
                XiuxianDateManage().update_user_hp_mp(
                    player1["user_id"], 1, int(player1["真元"] / (1 + user1_mp_buff))
                )
                #
                if player2["气血"] <= 0:
                    player2["气血"] = 1
        ...
        if user1_turn_skip == False and user2_turn_skip == False:
            play_list.append(
                {
                    "type": "node",
                    "data": {"name": "Bot", "uin": int(bot_id), "content": "双方都动弹不得！"},
                }
            )
            user1_turn_skip = True
            user2_turn_skip = True
        ...
        if player1["气血"] <= 0 or player2["气血"] <= 0:
            play_list.append(
                {
                    "type": "node",
                    "data": {"name": "Bot", "uin": int(bot_id), "content": "逻辑错误！"},
                }
            )
            break
...
async def Boss_fight(player1: dict, boss: dict, type_in=2, bot_id=0):
    ...
    if boss_js < 1:
        boss_js_data = {
            ...
            "data": {
                "name": f"{boss['name']}",
                "uin": int(bot_id),
                "content": f"凝聚真气,获得了{int((1-boss_js)*100)}%减伤!",
            },
        }
    ...
    while True:
        ...
        if player1_skil_open:  # 是否开启技能
            if user1_turn_skip:  # 无需跳过回合
                ...
                if player1_turn_cost == 0:  # 没有持续性技能生效
                    player1_js = player1_f_js  # 没有持续性技能生效,减伤恢复
                    if isEnableUserSikll(
                        player1, user1hpconst, user1mpcost, player1_turn_cost, skillrate
                    ):  # 满足技能要求，#此处为技能的第一次释放
                        skillmsg, user1_skill_sh, player1_turn_cost = get_skill_sh_data(
                            player1, user1_skill_date
                        )
                        if user1skill_type == 1:  # 直接伤害类技能
                            play_list.append(
                                get_msg_dict(player1, player_init_hp, skillmsg)
                            )
                            player1 = calculate_skill_cost(
                                player1, user1hpconst, user1mpcost
                            )
                            boss["气血"] = boss["气血"] - int(
                                user1_skill_sh * boss_js
                            )  # 玩家1的伤害 * boss的减伤
                            boss_hp_msg = f"{boss['name']}剩余血量{boss['气血']}"
                            play_list.append(
                                get_msg_dict(player1, player_init_hp, boss_hp_msg)
                            )
                            sh += user1_skill_sh
                            ...
                            if user1buff_type == 1:  # 攻击类buff
                                ...
                                play_list.append(
                                    get_msg_dict(
                                        player1, player_init_hp, player1_atk_msg
                                    )
                                )
                                boss["气血"] = boss["气血"] - int(
                                    player1_sh * boss_js
                        ...
                        player1_atk_msg = msg1.format(player1["道号"], player1_sh)
                        play_list.append(
                            get_msg_dict(player1, player_init_hp, player1_atk_msg)
                        )
                        boss["气血"] = boss["气血"] - int(
                            player1_sh * boss_js
                        )  # 玩家1的伤害 * 玩家2的减伤
                    ...
                    if user1skill_type == 2:  # 持续性伤害技能
                        player1_turn_cost = player1_turn_cost - 1
                        skillmsg = get_persistent_skill_msg(
                            player1["道号"],
                            user1_skill_date["name"],
                            user1_skill_sh,
                            player1_turn_cost,
                        ...
                        boss["气血"] = boss["气血"] - int(
                            (user1_skill_sh + player1_sh) * boss_js
                        )
                        boss_hp_msg = f"{boss['name']}剩余血量{boss['气血']}"
                        play_list.append(
                            get_msg_dict(player1, player_init_hp, boss_hp_msg)
                ...
                play_list.append(
                    get_msg_dict(
                        player1, player_init_hp, f"☆------{player1['道号']}动弹不得！------☆"
                    )
                )
                if player1_turn_cost > 0:
                    player1_turn_cost -= 1
            ...
            play_list.append(
                get_msg_dict(
                    player1, player_init_hp, f"☆------{player1['道号']}的回合------☆"
                )
            )
            isCrit, player1_sh = get_turnatk(player1)  # 判定是否暴击
            if isCrit:
        ...
        if boss_turn_skip:
            play_list.append(
                get_boss_dict(boss, qx, f"☆------{boss['name']}的回合------☆", bot_id)
            )
            ...
            if isCrit:
                ...
                msg2 = "{}发起攻击，造成了{}伤害\n"
            play_list.append(
                get_boss_dict(boss, qx, msg2.format(boss["name"], boss_sh), bot_id)
            )
            player1["气血"] = player1["气血"] - (boss_sh * player1_js)
            play_list.append(
                get_boss_dict(boss, qx, f"{player1['道号']}剩余血量{player1['气血']}", bot_id)
            )

        else:
            play_list.append(
                get_boss_dict(boss, qx, f"☆------{boss['name']}动弹不得！------☆", bot_id)
            )
...
def get_turnatk(player, buff=0):
    ...
    isCrit = False
    turnatk = int(
        round(random.uniform(0.95, 1.05), 2) * (player["攻击"] * (buff + 1))
    )  # 攻击波动,buff是攻击buff
...
def get_skill_hp_mp_data(player, secbuffdata):
    hpcost = (
        int(secbuffdata["hpcost"] * player["气血"]) if secbuffdata["hpcost"] != 0 else 0
    )
    mpcost = (
        int(secbuffdata["mpcost"] * player["exp"]) if secbuffdata["mpcost"] != 0 else 0
    )
