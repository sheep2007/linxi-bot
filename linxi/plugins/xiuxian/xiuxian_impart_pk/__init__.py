...
async def impart_pk_list_(bot: Bot, event: GroupMessageEvent):
    ...
    for x in range(len(xu_list)):
        ...
        list_msg.append(
            {
                "type": "node",
                "data": {"name": f"编号 {x}", "uin": bot.self_id, "content": msg},
            }
        )
    try:
...
async def impart_pk_now_(
    ...
    if impart_pk.find_user_data(user_info.user_id)["pk_num"] <= 0:
        ...
        await impart_pk_now.finish()
    if not num:
        msg, win = await impart_pk_uitls.impart_pk_now_msg_to_bot(
            user_info.user_name, NICKNAME
        )
        if win == 1:
        ...
        if win == 2:
            ...
            if (
                impart_pk.find_user_data(user_id)["pk_num"] <= 0
                and xu_world.check_xu_world_user_id(user_id) is True
            ):
    ...
    if num + 1 > len(xu_world_list) or num < 0:
        ...
        msg_list, win = await impart_pk_uitls.impart_pk_now_msg(
...
async def impart_pk_exp_(
    ...
    mainbuffratebuff = (
        mainbuffdata["ratebuff"] if mainbuffdata is not None else 0
    )  # 功法修炼倍率
    exp = int(
    ...
    if 0 < int(user_info.exp + exp) < max_exp:
        ...
        XiuxianDateManage().update_user_attribute(
            user_id, result_hp_mp[0], result_hp_mp[1], int(result_hp_mp[2] / 10)
        )
        msg = "虚神界修炼结束，共修炼{}分钟，本次闭关增加修为：{}{}{}".format(
            impaer_exp_time, exp, result_msg[0], result_msg[1]
        )
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(
