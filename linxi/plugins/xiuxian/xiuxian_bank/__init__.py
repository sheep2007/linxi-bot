...
bank = on_regex(
    r"^灵庄(存灵石|取灵石|升级会员|信息|结算)?(.*)?", priority=9, permission=GROUP, block=True
)
...
def get_give_stone(bankinfo):
    ...
    timedeff = round(
        (
            ...
            - datetime.strptime(savetime, "%Y-%m-%d %H:%M:%S")
        ).total_seconds()
        / 3600,
        2,
    )
    give_stone = int(
        bankinfo["savestone"] * timedeff * BANKLEVEL[bankinfo["banklevel"]]["interest"]
    )
