...
run_xiuxian = on_command("我要修仙", priority=8, permission=GROUP, block=True)
restart = on_command("重入仙途", permission=GROUP, priority=7, block=True)
sign_in = on_command("修仙签到", priority=13, permission=GROUP, block=True)
help_in = on_command("修仙帮助", priority=12, permission=GROUP, block=True)
# warring_help = on_fullmatch("风控帮助", priority=12, permission=GROUP, block=True)
rank = on_command(
    "排行榜",
    aliases={"修仙排行榜", "灵石排行榜", "战力排行榜", "境界排行榜", "宗门排行榜"},
    priority=7,
    permission=GROUP,
    block=True,
)
remaname = on_command("改名", priority=5, permission=GROUP, block=True)
level_up = on_command("突破", priority=6, permission=GROUP, block=True)
level_up_dr = on_command("渡厄突破", priority=7, permission=GROUP, block=True)
level_up_zj = on_command("直接突破", priority=7, permission=GROUP, block=True)
give_stone = on_command("送灵石", priority=5, permission=GROUP, block=True)
steal_stone = on_command(
    "偷灵石", aliases={"飞龙探云手"}, priority=4, permission=GROUP, block=True
)
...
open_xiuxian = on_command(
    "开启修仙", aliases={"关闭"}, permission=SUPERUSER, priority=5, block=True
)
user_leveluprate = on_command(
    "我的突破概率", aliases={"突破概率"}, priority=5, permission=GROUP, block=True
)
xiuxian_updata_level = on_command("修仙适配", priority=15, permission=GROUP, block=True)
xiuxian_uodata_data = on_command("更新记录", priority=15, permission=GROUP, block=True)
...
async def level_up_zj_(bot: Bot, event: GroupMessageEvent):
    ...
    if le == "失败":
        ...
        update_rate = (
            1
            if int(level_rate * XiuConfig().level_up_probability) <= 1
            else int(level_rate * XiuConfig().level_up_probability)
        )  # 失败增加突破几率
...
async def level_up_dr_(bot: Bot, event: GroupMessageEvent):
    ...
    if le == "失败":
        ...
        if pause_flag:
            ...
            sql_message.update_j_exp(user_id, now_exp)  # 更新用户修为
            nowhp = (
                user_msg.hp - (now_exp / 2) if (user_msg.hp - (now_exp / 2)) > 0 else 1
            )
            nowmp = user_msg.mp - now_exp if (user_msg.mp - now_exp) > 0 else 1
            sql_message.update_user_hp_mp(user_id, nowhp, nowmp)  # 修为掉了，血量、真元也要掉
            update_rate = (
            ...
            sql_message.update_levelrate(user_id, user_leveluprate + update_rate)
            msg = "没有检测到{}，道友突破失败,境界受损,修为减少{}，下次突破成功率增加{}%，道友不要放弃！".format(
                elixir_name, now_exp, update_rate
            )
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(
...
async def rob_stone_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if give_qq:
        ...
        if user_2:
            ...
            player1["会心"] = int(
                (
                    0.01 + impart_data_1.impart_know_per
                    if impart_data_1 is not None
                    else 0
                )
                * 100
            )
            ...
            player2["会心"] = int(
                (
                    0.01 + impart_data_2.impart_know_per
                    if impart_data_2 is not None
                    else 0
                )
                * 100
            )
            ...
            if victor == player1["道号"]:
                ...
                if mind_stone > 0:
                    ...
                    msg = "大战一番，被对手反杀，损失灵石{}枚，修为减少{}，对手获取灵石{}枚，修为增加{}".format(
                        int(mind_stone * 0.1), exps, int(mind_stone * 0.1), exps / 2
                    )
                    if XiuConfig().img:
                        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
                        await bot.send_group_msg(
