...
create_rift = on_fullmatch(
    "生成秘境", priority=5, permission=GROUP and SUPERUSER, block=True
)
complete_rift = on_command(
    "秘境结算", aliases={"结算秘境"}, priority=7, permission=GROUP, block=True
)
break_rift = on_command(
    "秘境探索终止", aliases={"终止探索秘境"}, priority=7, permission=GROUP, block=True
)
        ...
        if exp_time < time2:
            ...
            if rift_type == "无事":
                ...
                if rift_type == "掉血事件":
                    ...
                    result, msg = await get_boss_battle_info(
                        user_info, rift_rank, bot.self_id
                    )
