...
def get_weapon_info_msg(weapon_id, weapon_info=None):
    ...
    atk_buff_msg = (
        f"提升{int(weapon_info['atk_buff'] * 100)}%攻击力！"
        if weapon_info["atk_buff"] != 0
        else ""
    )
    crit_buff_msg = (
        f"提升{int(weapon_info['crit_buff'] * 100)}%会心率！"
        if weapon_info["crit_buff"] != 0
        else ""
    )
...
def get_main_info_msg(id):
    ...
    hpmsg = (
        f"提升{round(mainbuff['hpbuff'] * 100, 0)}%气血" if mainbuff["hpbuff"] != 0 else ""
    )
    mpmsg = (
        f"提升{round(mainbuff['mpbuff'] * 100, 0)}%真元" if mainbuff["mpbuff"] != 0 else ""
    )
    atkmsg = (
        f"提升{round(mainbuff['atkbuff'] * 100, 0)}%攻击力"
        if mainbuff["atkbuff"] != 0
        else ""
    )
    ratemsg = (
        f"提升{round(mainbuff['ratebuff'] * 100, 0)}%修炼速度"
        if mainbuff["ratebuff"] != 0
        else ""
    )
...
def get_sec_msg(secbuffdata):
    ...
    if secbuffdata is None:
        ...
        return msg
    hpmsg = (
        f"，消耗当前血量{int(secbuffdata['hpcost'] * 100)}%"
        if secbuffdata["hpcost"] != 0
        else ""
    )
    mpmsg = (
        f"，消耗真元{int(secbuffdata['mpcost'] * 100)}%"
