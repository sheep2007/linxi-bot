...
async def draw_user_info_img(user_id, DETAIL_MAP):
    ...
    try:
        ...
        if scaled_h < based_h:  # 缩放后图片不够高（横屏图）
            ...
            img = img.resize((img_w, based_h)).crop(
                (crop_l, 0, crop_l + based_w, based_h)
            )
        else:
            img_h = scaled_h
            crop_t = round((img_h / 2) - (based_h / 2))
            img = img.resize((based_w, img_h)).crop(
                (0, crop_t, based_w, crop_t + based_h)
            )
        ...
        img = (
            Image.open(TEXT_PATH / "back.png")
            .resize((based_w, based_h))
            .convert("RGBA")
        )
    # 获取用户头像圆框
    user_status = (
        Image.open(TEXT_PATH / "user_state.png").resize((450, 450)).convert("RGBA")
    )
    ...
    DETAIL_sectinfo = {"所在宗门": DETAIL_MAP["所在宗门"], "宗门职位": DETAIL_MAP["宗门职位"]}
    tasks3 = []
