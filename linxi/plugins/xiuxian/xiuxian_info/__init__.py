...
xiuxian_message = on_command(
    "我的修仙信息", aliases={"我的存档"}, priority=23, permission=GROUP, block=True
)
sql_message = XiuxianDateManage()  # sql类


@xiuxian_message.handle(
    parameterless=[Cooldown(cd_time=XiuConfig().user_info_cd, at_sender=True)]
)
async def xiuxian_message_(bot: Bot, event: GroupMessageEvent):
    ...
    if user_main_buff_date is not None:
        main_buff_name = (
            f"{user_main_buff_date['name']}({user_main_buff_date['level']})"
        )
    ...
    if XiuConfig().user_info_image:
        ...
        await bot.send_group_msg(
            group_id=int(send_group_id), message=MessageSegment.image(img_res)
        )
        await xiuxian_message.finish()
    else:
        try:
