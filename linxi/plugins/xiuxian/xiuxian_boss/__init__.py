...
def check_rule_bot_boss() -> Rule:  # 对传入的消息检测，是主qq传入的消息就响应，其他的不响应
    async def _check_bot_(bot: Bot, event: GroupMessageEvent) -> bool:
        if (
            ...
            or event.get_user_id() in del_boss_id
        ):
            return True
        else:
            return False
...
create = on_command(
    "生成世界boss",
    aliases={"生成世界Boss", "生成世界BOSS"},
    priority=5,
    permission=GROUP and SUPERUSER,
    block=True,
)
boss_info = on_command(
    "查询世界boss",
    aliases={"查询世界Boss", "查询世界BOSS"},
    priority=6,
    permission=GROUP,
    block=True,
)
set_group_boss = on_command(
    "世界boss",
...
battle = on_command(
    "讨伐boss",
    aliases={"讨伐世界boss", "讨伐Boss", "讨伐BOSS", "讨伐世界Boss", "讨伐世界BOSS"},
    priority=6,
    permission=GROUP,
    block=True,
)
boss_help = on_command(
    "世界boss帮助", aliases={"世界Boss帮助", "世界BOSS帮助"}, priority=5, block=True
)
boss_delete = on_command(
    "天罚boss",
    aliases={"天罚世界boss", "天罚Boss", "天罚BOSS", "天罚世界Boss", "天罚世界BOSS"},
    priority=7,
    rule=check_rule_bot_boss(),
    block=True,
)
boss_integral_info = on_command(
    "世界积分查看",
    aliases={"查看世界积分", "查询世界积分", "世界积分查询"},
    priority=10,
    permission=GROUP,
    block=True,
)
...
async def set_boss_():
    ...
    try:
        for group_id in groups_list:
            ...
            logger.success(
                "开启群{}boss,每{}小时{}分钟刷新！".format(
                    group_id,
                    groups[str(group_id)]["hours"],
                    groups[str(group_id)]["minutes"],
                )
            )
    ...
    if session_id in cache_help:
        ...
        if str(send_group_id) in groups:
            msg = __boss_help__ + "非指令:1、拥有定时任务:每{}小时{}分钟生成一只随机大境界的世界Boss".format(
                groups[str(send_group_id)]["hours"],
                groups[str(send_group_id)]["minutes"],
            )
        else:
            msg = __boss_help__
        if XiuConfig().img:
...
async def battle_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    result, victor, bossinfo_new, get_stone = await Boss_fight(
        player, bossinfo, bot_id=bot.self_id
    )
    if victor == "Boss赢了":
    ...
    if is_in:
        ...
        if user_boss_fight_info["boss_integral"] < cost:
            ...
            sql_message.send_back(
                user_id, shop_id, item_info["name"], item_info["type"], 1
            )
