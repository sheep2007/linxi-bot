...
from ..utils import check_user, send_forward_msg, get_msg_pic, CommandObjectID
...
yaocai_get = on_command(
    "灵田收取", aliases={"灵田结算"}, priority=8, permission=GROUP, block=True
)
...
async def mix_elixir_sqdj_up_(bot: Bot, event: GroupMessageEvent):
    ...
    msg = (
        f"道友的收取等级目前为：{mix_elixir_info['收取等级']}级，可以使灵田收获的药材增加{mix_elixir_info['收取等级']}个！"
    )
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
        await bot.send_group_msg(
...
async def yaocai_get_(bot: Bot, event: GroupMessageEvent):
    ...
    if int(user_info.blessed_spot_flag) == 0:
        ...
        await yaocai_get.finish()
    mix_elixir_info = get_player_info(user_id, "mix_elixir_info")
    GETCONFIG = {"time_cost": 48, "加速基数": 0.05}  # 单位小时
            ...
            * (1 - (GETCONFIG["加速基数"] * mix_elixir_info["药材速度"])),
            ...
            if not yaocai_id_list:
                ...
                for k, v in give_dict.items():
                    ...
                    sql_message.send_back(
                        user_info.user_id, k, goods_info["name"], "药材", v
                    )
...
async def mix_elixir_(bot: Bot, event: GroupMessageEvent):
    ...
    if finall_mix_elixir_msg == {}:
        ...
        ldl_name = sorted(
            user_ldl_dict[user_id].items(), key=lambda x: x[0], reverse=False
        )[0][1]
        l_msg = []
        for k, v in finall_mix_elixir_msg.items():
            ...
            if v["配方"]["辅药_num"] != 0:
                msg += (
                    f"辅药：{v['配方']['辅药']},{v['配方']['辅药_level']}，数量：{v['配方']['辅药_num']}\n"
                )
            l_msg.append(msg)
        if len(l_msg) > 51:
            l_msg = l_msg[:50]
...
async def _mix_elixir(bot: Bot, event: GroupMessageEvent, mode: str = EventPlainText()):
    ...
    if matched is None:
        ...
        check, zhuyao_goods_id = await check_yaocai_name_in_back(
            user_id, zhuyao_name, zhuyao_num
        )
        if not check:
        ...
        check, yaoyin_goods_id = await check_yaocai_name_in_back(
            user_id, yaoyin_name, yaoyin_num
        )
        if not check:
        ...
        check, fuyao_goods_id = await check_yaocai_name_in_back(
            user_id, fuyao_name, fuyao_num
        )
        if not check:
        ...
        if zhuyao_name == fuyao_name:
            check, fuyao_goods_id = await check_yaocai_name_in_back(
                user_id, fuyao_name, fuyao_num + zhuyao_num
            )
            if not check:
                msg = f"请检查药材：{zhuyao_name} 是否在背包中，或者数量是否足够！"
                if XiuConfig().img:
        ...
        if yaoyin_name == fuyao_name:
            check, fuyao_goods_id = await check_yaocai_name_in_back(
                user_id, fuyao_name, fuyao_num + yaoyin_num
            )
            if not check:
                msg = f"请检查药材：{yaoyin_name} 是否在背包中，或者数量是否足够！"
                if XiuConfig().img:
        ...
        if await tiaohe(zhuyao_info, zhuyao_num, yaoyin_info, yaoyin_num):  # 调和失败
            ...
            elixir_config[str(fuyao_info["辅药"]["type"])] = (
                fuyao_info["辅药"]["power"] * fuyao_num
            )
            ...
            if is_mix:
                ...
                impart_data = xiuxian_impart.get_user_message(user_id)
                impart_mix_per = (
                    impart_data.impart_mix_per if impart_data is not None else 0
                )
