...
async def last_work_(bot: Bot, event: GroupMessageEvent):
    ...
    if (
        (is_type and USERRANK[user_info.level] <= 22)
        or (is_type and user_info.exp >= sql_message.get_level_power("虚道境圆满"))
        or (
            is_type
            and int(user_info.exp)
            >= int(OtherSet().set_closing_type(user_level))
            ...
            if big_suc:  # 大成功
                ...
                if item_flag:
                    sql_message.send_back(
                        user_id, item_id, item_info["name"], item_info["type"], 1
                    )
...
async def do_work_(
    bot: Bot, event: GroupMessageEvent, args: Tuple[Any, ...] = RegexGroup()
):
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    user_level = "虚道境圆满"
    isUser, user_info, msg = check_user(event)
    ...
    user_cd_message = sql_message.get_user_cd(user_id)
    if (
        not os.path.exists(PLAYERSDATA / str(user_id) / "workinfo.json")
        and user_cd_message.type == 2
    ):
        sql_message.do_work(user_id, 0)
    ...
    if mode is None:  # 接取逻辑
        if (user_cd_message.scheduled_time is None) or (user_cd_message.type == 0):
            ...
            exp_time = (datetime.now() - work_time).seconds // 60  # 时长计算
            time2 = workhandle().do_work(
                key=1, name=user_cd_message.scheduled_time, user_id=user_info.user_id
            )
            if exp_time < time2:
    ...
    if mode == "刷新":  # 刷新逻辑
        ...
        work_msg = workhandle().do_work(
            0, level=user_level, exp=user_info.exp, user_id=user_id
        )
        n = 1
        work_list = []
        ...
        for i in work_msg:
            ...
            n += 1
        work_msg_f += (
            f"(悬赏令每日免费刷新次数：{count}，超过{count}次后，下次刷新消耗灵石{lscost},今日可免费刷新次数：{freenum}次)"
        )
        if int(stone_use) == 1:
