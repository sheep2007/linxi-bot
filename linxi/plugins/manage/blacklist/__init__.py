...
def blacklist_processor(event: Event):
    if (uid := str(vars(event).get("user_id", None))) in superusers:
        return
    if (gid := str(vars(event).get("group_id", None))) and gid in blacklist[
        "grouplist"
    ]:
...
add_userlist = on_command(
    "拉黑用户", aliases={"屏蔽用户"}, permission=SUPERUSER, priority=1, block=True
)
...
add_grouplist = on_command(
    "拉黑群", aliases={"屏蔽群"}, permission=SUPERUSER, priority=1, block=True
)
...
del_userlist = on_command(
    "解禁用户", aliases={"解封用户"}, permission=SUPERUSER, priority=1, block=True
)
...
del_grouplist = on_command(
    "解禁群", aliases={"解封群"}, permission=SUPERUSER, priority=1, block=True
)
...
async def check_user_list():
    await check_userlist.finish(
        f"当前已屏蔽 {len(blacklist['userlist'])} 个用户: {', '.join(blacklist['userlist'])}"
    )
...
async def check_group_list():
    await check_grouplist.finish(
        f"当前已屏蔽 {len(blacklist['grouplist'])} 个群聊: {', '.join(blacklist['grouplist'])}"
    )
...
add_all_group = on_command(
    "拉黑所有群", aliases={"屏蔽所有群"}, permission=SUPERUSER, priority=1, block=True
)
...
del_all_group = on_command(
    "解禁所有群", aliases={"解封所有群"}, permission=SUPERUSER, priority=1, block=True
)
...
add_all_friend = on_command(
    "拉黑所有好友", aliases={"屏蔽所有好友"}, permission=SUPERUSER, priority=1, block=True
)
...
del_all_friend = on_command(
    "解禁所有好友", aliases={"解封所有好友"}, permission=SUPERUSER, priority=1, block=True
)
...
reset_blacklist = on_command(
    "重置黑名单", aliases={"清空黑名单"}, permission=SUPERUSER, priority=1, block=True
)
...
async def _(bot: Bot, event: GroupBanNoticeEvent):
    if (
        "ban_auto_sleep" in blacklist["grouplist"]
        and event.is_tome()
        and event.duration
    ):
        handle_blacklist([f"{event.group_id}"], "add", "grouplist")
        for superuser in bot.config.superusers:
            await bot.send_private_msg(
                user_id=int(superuser), message=f"ⓘ在群聊 {event.group_id} 受到禁言, 已自动拉黑该群聊."
            )
...
async def _(arg: Message = CommandArg()):
    ...
    if not msg or msg.startswith("开"):
        ...
        blacklist["grouplist"] = [
            uid for uid in blacklist["grouplist"] if uid not in ["ban_auto_sleep"]
        ]
        text = "自觉静默已关闭."
    else:
        return
