...
def MsgText(data: str):
    ...
    try:
        ...
        # 拼接成字符串并且去除两端空格
        msg_text = " ".join(
            map(lambda x: x["data"]["text"].strip(), msg_text_list)
        ).strip()
        return msg_text
    except:
        return ""
...
async def init():
    ...
    if not config_admin.exists():
        await mk(
            "file", config_admin, "w", content='{"1008611": ["This_is_an_example"]}'
        )
...
async def banSb(gid: int, ban_list: list, time: int = None, scope: list = None):
    ...
    """
    if "all" in ban_list:
        yield nonebot.get_bot().set_group_whole_ban(group_id=gid, enable=True)
    else:
        if time is None:
            if scope is None:
                time = random.randint(
...
async def change_s_title(
    bot: Bot, matcher: Matcher, gid: int, uid: int, s_title: Optional[str]
):
    """
    改头衔
    :param bot: bot
...
async def get_user_violation(
    gid: int, uid: int, label: str, content: str, add_: bool = True
) -> int:
    """
    获取用户违规情况
    :param gid: 群号
...
async def vio_level_init(path_user, uid, this_time, label, content) -> None:
    with open(path_user, mode="w", encoding="utf-8") as c:
        c.write(
            json.dumps(
                {uid: {"level": 0, "info": {this_time: [label, content]}}},
                ensure_ascii=False,
            )
        )
