...
class PluginManager:
    ...
    def __init__(self, path: Path = Path() / "data" / "plugin_manage" / "plugin_channel_list.yml"):
        self.__path = path
        self.__load()

    def get_plugin(
        self, conv: Conv = {"user": [], "channel": []}, perm: Union[str, int] = 5
    ) -> Dict[str, bool]:
        """
        获取指定会话的插件列表
        ...
        """
        result = {}
        type = len(conv["user"]) + len(conv["channel"])
