...
from nonebot_plugin_guild_patch import GUILD, GuildMessageEvent, Message
...
if leaf.leaf_permission == "GROUP":
    permission = GROUP
elif leaf.leaf_permission == "GUILD":
    permission = GUILD
else:
    permission = None
...
if interrupt > -1:
    ...
    async def repeat(event: GroupMessageEvent) -> bool:
        ...
        if msg_last.get(group_id) == msg:
            repeater_times.setdefault(
                group_id, random.randint(repeater_limit[0], repeater_limit[1])
            )
            msg_times[group_id] += 1
            if msg_times[group_id] == repeater_times[group_id]:
                repeater_times[group_id] = random.randint(
                    repeater_limit[0], repeater_limit[1]
                )
    ...
    repeater = on_message(
        rule=repeat, permission=GROUP | GUILD, priority=99, block=False
    )
