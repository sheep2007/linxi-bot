...
详情请见文档。
""".strip(),
    extra={"unique_name": "bottle"},
)

throw = on_command(
    "扔漂流瓶 ",
    aliases=set(["bottle throw ", "丢漂流瓶 "]),
    permission=GROUP,
    priority=100,
    block=True,
)
...
comment = on_command(
    "评论漂流瓶 ", aliases=set(["bottle comment"]), priority=100, block=True
)
    ...
    if not bottle.select():
        ...
        try:
            user = await bot.get_group_member_info(
                group_id=data["group"], user_id=data["user"]
            )
            user = user["nickname"]
        except:
            user = data["user_name"]
        ...
        ba.add("cooldown", event.user_id)
        await get.finish(
            f'【漂流瓶No.{bott[0]}|被捡到{data["picked"]}次】来自【{group}】的 {user} ！\n'
            + Message(data["text"])
...
async def rep(bot: Bot, event: GroupMessageEvent):
    if (not ba.verify(event.user_id, event.group_id)) or (
        not ba.verifyReport(event.user_id)
    ):
        await throw.finish(ba.bannedMessage)
...
async def com(bot: Bot, event: GroupMessageEvent):
    ...
    try:
        await bot.send_msg(
            group_id=bottle.check_bottle(index)["group"],
            message=Message(
...
async def rem(bot: Bot, event: GroupMessageEvent):
    ...
    if (
        ...
        or bottle.check_bottle(index)["user"] == event.user_id
    ):
