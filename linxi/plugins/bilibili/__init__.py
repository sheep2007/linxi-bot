...
__plugin_meta__ = PluginMetadata(
    name="Bilibili",
    description="将up主更新、番剧更新、主播开播等信息推送到QQ",
    usage=__usage__,
    config=Config,
    extra={"unique_name": "bilibili", "example": "/bili 关注动态 2019044357"},
)


__PLUGIN_NAME = "[Linxi-Bilibili]"

ALL_PERMISSION = GROUP_ADMIN | GROUP_OWNER | PRIVATE_FRIEND | SUPERUSER
PACKAGEPATH = dirname(abspath(__file__))

follow_liver_command = on_command(
    "bili 关注主播", aliases={"bili follow_anchor"}, permission=ALL_PERMISSION
)


@follow_liver_command.handle()
async def follow_liver_command_handler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    uid_list = args.extract_plain_text().split()
    await create_user(event)
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await follow_liver_list(
            event.sender.user_id, uid_list, 0
        )
...
unfollow_liver_command = on_command(
    "bili 取关主播", aliases={"bili unfollow_anchor"}, permission=ALL_PERMISSION
)


@unfollow_liver_command.handle()
async def unfollow_liver_command_handler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    uid_list = args.extract_plain_text().split()
    await create_user(event)
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await unfollow_liver_list(
            event.sender.user_id, uid_list, 0
        )
    if isinstance(event, GroupMessageEvent):
...
listFollowingCommand = on_command(
    "bili 查询关注", aliases={"bili 查询成分", "bili list"}, permission=ALL_PERMISSION
)


@listFollowingCommand.handle()
async def listFollowingCommandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    ...
    try:
        ...
        if res:
            if user_type == 0:
                ...
                logger.debug(
                    f"{__PLUGIN_NAME}followed_telegram_list = {followed_telegram_list}"
                )
            ...
            if "直播" in inputArgs:
                if followed_liver_list:
                    ...
                    for liver_uid in followed_liver_list:
                        liver_uid, liver_name, _, _ = bili_database.query_info(
                            3, liver_uid
                        )
            ...
            if "番剧" in inputArgs:
                if followed_telegram_list:
                    ...
                    for season_id in followed_telegram_list:
                        season_id, telegram_title, _, _ = bili_database.query_info(
                            4, season_id
                        )
                        textMsg += (
                            "> " + f"{telegram_title}(season_id: ss{season_id})" + "\n"
                        )
...
followUpCommand = on_command(
    "bili 关注up", aliases={"bili 关注UP", "bili follow_up"}, permission=ALL_PERMISSION
)


@followUpCommand.handle()
async def followUpCommandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    uid_list = args.extract_plain_text().split()
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await follow_up_list(
            event.sender.user_id, uid_list, 0
        )
    if isinstance(event, GroupMessageEvent):
...
unfollowUpCommand = on_command(
    "bili 取关up", aliases={"bili 取关UP", "bili unfollow_up"}, permission=ALL_PERMISSION
)


@unfollowUpCommand.handle()
async def unfollowUpCommandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    uid_list = args.extract_plain_text().split()
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await unfollow_up_list(
            event.sender.user_id, uid_list, 0
        )
    if isinstance(event, GroupMessageEvent):
...
followTelegramCommand = on_command(
    "bili 关注番剧", aliases={"bili follow_anime"}, permission=ALL_PERMISSION
)


@followTelegramCommand.handle()
async def followTelegramCommandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    tele_id_list = args.extract_plain_text().split()
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await follow_telegram_list(
            event.sender.user_id, tele_id_list, 0
        )
    if isinstance(event, GroupMessageEvent):
        success_list, fail_list = await follow_telegram_list(
            event.group_id, tele_id_list, 1
        )
    await followTelegramCommand.finish(f"关注成功:\n{success_list}\n关注失败:\n{fail_list}")


unfollowTelegramCommand = on_command(
    "bili 取关番剧", aliases={"bili unfollow_anime"}, permission=ALL_PERMISSION
)


@unfollowTelegramCommand.handle()
async def unfollowTelegramCommandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    season_id_list = args.extract_plain_text().split()
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await unfollow_telegram_list(
            event.sender.user_id, season_id_list, 0
        )
    if isinstance(event, GroupMessageEvent):
        success_list, fail_list = await unfollow_telegram_list(
            event.group_id, season_id_list, 1
        )
    await unfollowTelegramCommand.finish(f"取关成功:\n{success_list}\n取关失败:\n{fail_list}")


follow_dynamic_command = on_command(
    "bili 关注动态", aliases={"bili follow_action"}, permission=ALL_PERMISSION
)


@follow_dynamic_command.handle()
async def follow_dynamic_commandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    uid_list = args.extract_plain_text().split()
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await follow_dynamic_list(
            event.sender.user_id, uid_list, 0
        )
    if isinstance(event, GroupMessageEvent):
...
unfollow_dynamic_command = on_command(
    "bili 取关动态", aliases={"bili unfollow_action"}, permission=ALL_PERMISSION
)


@unfollow_dynamic_command.handle()
async def unfollow_dynamic_commandHandler(
    event: Union[PrivateMessageEvent, GroupMessageEvent], args: Message = CommandArg()
):
    await create_user(event)
    uid_list = args.extract_plain_text().split()
    if isinstance(event, PrivateMessageEvent):
        success_list, fail_list = await unfollow_dynamic_list(
            event.sender.user_id, uid_list, 0
        )
    if isinstance(event, GroupMessageEvent):
        success_list, fail_list = await unfollow_dynamic_list(
            event.group_id, uid_list, 1
        )
...
followUpByShare = on_message(
    rule=regex(
        '\[CQ:json,[\w\W]*"appid":100951776[\w\W]*space.bilibili.com[\w\W]*[\w\W]*\]'
    )
    & privateMessageRule,
...
async def streamerShareHandler(event: PrivateMessageEvent):
    ...
    try:
        ...
        success_list, fail_list = await follow_liver_list(
            event, event.sender.user_id, [uid], 0
        )
...
followTelegramByShare = on_message(
    rule=regex(
        '^\[CQ:json[\w\W]*"appid":100951776[\w\W]*www.bilibili.com\/bangumi\/play\/[\w\W]*'
    )
    & privateMessageRule,
    permission=PRIVATE_FRIEND,
)
...
scheduler.add_job(
    check_bili_live, "interval", minutes=1, id="bili_stream", misfire_grace_time=90
)
scheduler.add_job(
    check_up_update, "interval", minutes=2, id="bili_up", misfire_grace_time=90
)
scheduler.add_job(
