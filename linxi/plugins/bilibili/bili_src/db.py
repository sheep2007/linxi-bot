...
db_root = Path().absolute() / "data" / "bilibili"

db = db_root / "bilibili_2.db"
...
class BiliDatabase:
    ...
    def init_database(self) -> None:
        ...
        # 创建关注up表
        cur.execute(
            """CREATE TABLE IF NOT EXISTS up_follower
            (
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS up_group_index ON up_follower (group_id)"
        )

        # 创建关注主播表
        cur.execute(
            """CREATE TABLE IF NOT EXISTS liver_follower
            (
                id INTERGER PRIMARY KEY,
        ...
        # 创建索引
        cur.execute(
            "CREATE INDEX IF NOT EXISTS liver_index ON liver_follower (liver_uid)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS liver_user_index ON liver_follower (user_id)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS liver_group_index ON liver_follower (group_id)"
        )

        # 创建关注节目表
        cur.execute(
            """CREATE TABLE IF NOT EXISTS telegram_follower
            (
                id INTERGER PRIMARY KEY,
        ...
        # 创建索引
        cur.execute(
            "CREATE INDEX IF NOT EXISTS tele_index ON telegram_follower (season_id)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS tele_user_index ON telegram_follower (user_id)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS tele_group_index ON telegram_follower (group_id)"
        )

        # 创建关注动态表
        cur.execute(
            """CREATE TABLE IF NOT EXISTS dynamic_follower
            (
                id INTERGER PRIMARY KEY,
    ...
    def query_specified_realtion(
        self, sql_type: int, user_id: str, target_id: str
    ) -> bool:
        """查询一对一的关注
    ...
    def dbg_check_telegram_follow(self) -> List[Tuple]:
        ...
        try:
            cur.execute(
                "SELECT rowid, season_id, user_id, group_id FROM telegram_follower"
            )
        except Exception as e:
            ##logger.error(f'查询群组关注时发生错误:\n{e}')
            raise BiliDatebaseError(f"数据库查询群组关注时发生错误:{e.args[0]}")
