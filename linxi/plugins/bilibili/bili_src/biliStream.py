...
__PLUGIN_NAME = "[Linxi-Bilibili~直播]"
...
async def check_bili_live() -> None:
    ...
    results = await asyncio.gather(
        *[
            bili_client.get_live_status(liver_info["liver_uid"], liver_info["room_id"])
            for liver_info in liver_list
        ],
        return_exceptions=True,
    )
    for i in range(len(liver_list)):
        if isinstance(results[i], tuple):
            if results[i][0] and not liver_list[i]["is_live"]:
                logger.info(
                    f'[{__PLUGIN_NAME}]检测到主播 <{liver_list[i]["liver_name"]}> 已开播！'
                )
                text_msg = "【直播动态】\n<{}>正在直播!\n标题: {}\n链接: {}".format(
                    liver_list[i]["liver_name"],
                    results[i][1],
                    f"https://live.bilibili.com/{liver_list[i]['room_id']}",
                )
                ...
                logger.info(
                    f'[{__PLUGIN_NAME}]检测到主播 <{liver_list[i]["liver_name"]}> 已下播'
                )
...
async def follow_liver(uid: str, user_id: str, user_type: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if (
            user_type == 0
            and user_id in bili_task_manager.liver_list[uid]["user_follower"]
            or user_type == 1
            and user_id in bili_task_manager.liver_list[uid]["group_follower"]
        ):
            logger.debug(
                f'{__PLUGIN_NAME}主播 <{bili_task_manager.liver_list[uid]["liver_name"]}> 已关注'
            )
        ...
        logger.info(
            f"{__PLUGIN_NAME}用户/群 <{user_id}> 关注主播 <{bili_task_manager.liver_list[uid]['liver_name']}> 成功"
        )
...
async def unfollower_liver(uid: str, user_id: str, user_type: int) -> Tuple[bool, str]:
    ...
    try:
        # 处理未关注
        if (
            uid not in bili_task_manager.liver_list
            or user_type == 0
            and user_id not in bili_task_manager.liver_list[uid]["user_follower"]
            or user_type == 1
            and user_id not in bili_task_manager.liver_list[uid]["group_follower"]
        ):
