...
class BiliTaskManager:
        ...
        try:
            ...
            for up_uid, up_name, latest_timestamp in up_list:
                self.up_list[up_uid] = {
                    ...
                    "group_follower": set(
                        bili_database.query_group_relation(0, up_uid)
                    ),
                }
                if (
                    not self.up_list[up_uid]["user_follower"]
                    and not self.up_list[up_uid]["group_follower"]
                ):
            ...
            for liver_uid, liver_name, is_live, room_id in liver_list:
                self.liver_list[liver_uid] = {
                    ...
                    "room_id": room_id,
                    "user_follower": set(
                        bili_database.query_user_relation(2, liver_uid)
                    ),
                    "group_follower": set(
                        bili_database.query_group_relation(2, liver_uid)
                    ),
                }
                if (
                    not self.liver_list[liver_uid]["user_follower"]
                    and not self.liver_list[liver_uid]["group_follower"]
                ):
            ...
            for season_id, telegram_title, episode, is_finish in telegram_list:
                ...
                self.telegram_list[season_id] = {
                    ...
                    "user_follower": set(
                        bili_database.query_user_relation(4, season_id)
                    ),
                    "group_follower": set(
                        bili_database.query_group_relation(4, season_id)
                    ),
                }
                if (
                    not self.telegram_list[season_id]["user_follower"]
                    and not self.telegram_list[season_id]["group_follower"]
                ):
            ...
            for uid, u_name, pin_id_str, latest_timestamp in dynamic_list:
                ...
                if (
                    not self.dynamic_list[uid]["user_follower"]
                    and not self.dynamic_list[uid]["group_follower"]
                ):
    ...
    def get_up_check_update_list(self) -> List[str]:
        ...
        for up_uid in self.up_list.keys():
            if (
                self.up_list[up_uid]["next_update_check"]
                == self.__up_update_check_ptr__
            ):
                check_update_list.append(up_uid)
        self.__up_update_check_ptr__ = (
            self.__up_update_check_ptr__ + 1
        ) % self.__update_check_len__
    ...
    def get_dynamic_check_update_list(self) -> List[str]:
        ...
        for uid in self.dynamic_list.keys():
            if (
                self.dynamic_list[uid]["next_update_check"]
                == self.__dynamic_update_check_ptr__
            ):
                check_update_list.append(uid)
        self.__dynamic_update_check_ptr__ = (
            self.__dynamic_update_check_ptr__ + 1
        ) % self.__update_check_len__
        return check_update_list
    ...
    def update_telegram_info(
        self, season_id: str, episode: int, is_finish: bool
    ) -> bool:
        """更新任务管理器以及数据库中的番剧信息
        ...
        try:
            ...
            if target_type == 0:
                if target_id not in self.up_list:
                    _, up_name, latest_timestamp = bili_database.query_info(
                        2, target_id
                    )
                    self.up_list[target_id] = {
                        ...
                        "latest_timestamp": latest_timestamp,
                        "user_follower": set(
                            bili_database.query_user_relation(0, target_id)
                        ),
                        "group_follower": set(
                            bili_database.query_group_relation(0, target_id)
                        ),
                ...
                if target_id not in self.liver_list:
                    _, liver_name, is_live, room_id = bili_database.query_info(
                        3, target_id
                    )
                    self.liver_list[target_id] = {
                        ...
                        "room_id": room_id,
                        "user_follower": set(
                            bili_database.query_user_relation(2, target_id)
                        ),
                        "group_follower": set(
                            bili_database.query_group_relation(2, target_id)
                        ),
                    }
                else:
                ...
                if target_id not in self.telegram_list:
                    _, telegram_title, episode, is_finish = bili_database.query_info(
                        4, target_id
                    )
                    self.telegram_list[target_id] = {
                        ...
                        "is_finish": bool(is_finish),
                        "user_follower": set(
                            bili_database.query_user_relation(4, target_id)
                        ),
                        "group_follower": set(
                            bili_database.query_group_relation(4, target_id)
                        ),
                    }
                else:
                ...
                if target_id not in self.dynamic_list:
                    _, u_name, pin_id_str, latest_timestamp = bili_database.query_info(
                        5, target_id
                    )
                    self.dynamic_list[target_id] = {
                        ...
                        "user_follower": set(
                            bili_database.query_user_relation(6, target_id)
                        ),
                        "group_follower": set(
                            bili_database.query_group_relation(6, target_id)
                        ),
                    }
                else:
    ...
    def add_group_follower(
        self, target_type: int, target_id: str, group_id: str
    ) -> bool:
        """_summary_
    ...
    def remove_user_follower(
        self, target_type: int, target_id: str, user_id: str
    ) -> bool:
        """移除个人用户的关注
        ...
        try:
            ...
            if target_type == 0:
                ...
                if (
                    not self.up_list[target_id]["user_follower"]
                    and not self.up_list[target_id]["group_follower"]
                ):
                ...
                if (
                    not self.liver_list[target_id]["user_follower"]
                    and not self.liver_list[target_id]["group_follower"]
                ):
                ...
                if (
                    not self.telegram_list[target_id]["user_follower"]
                    and not self.telegram_list[target_id]["group_follower"]
                ):
                ...
                if (
                    not self.dynamic_list[target_id]["user_follower"]
                    and not self.dynamic_list[target_id]["group_follower"]
                ):
    ...
    def add_liver_info(
        self, liver_uid: str, liver_name: str, is_live: bool, room_id: str
    ) -> bool:
        """向任务管理器以及数据库中增加主播信息
    ...
    def add_telegram_info(
        self, season_id: str, telegram_title: str, episode: int, is_finish: bool
    ) -> bool:
        """向任务管理器以及数据库中增加番剧信息
        ...
        try:
            if not bili_database.query_info(4, season_id):
                bili_database.insert_info(
                    4, season_id, telegram_title, episode, is_finish
                )
            self.telegram_list[season_id] = {
                "season_id": season_id,
                "telegram_title": telegram_title,
    ...
    def add_dynamic_info(
        self, uid: str, u_name: str, pin_id_str: str, latest_timestamp: int
    ) -> bool:
        """向任务管理器以及数据库中增加动态主信息
