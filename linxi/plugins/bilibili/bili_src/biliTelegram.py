...
__PLUGIN_NAME = "[Linxi-Bilibili~番剧]"
...
async def check_telegram_update():
    ...
    results = await asyncio.gather(
        *[
            bili_client.get_telegram_latest_episode(
                telegram_info["season_id"], telegram_info["episode"]
            )
            for telegram_info in telegram_list
        ],
    ...
    for i in range(len(telegram_list)):
        if isinstance(results[i], tuple):
            if results[i][0] is True:
                logger.info(
                    f'[{__PLUGIN_NAME}]检测到影视剧 <{telegram_list[i]["telegram_title"]}> 更新'
                )
                text_msg = "【B站动态】\n《{}》已更新第{}集\n标题: {}\n链接: {}\n".format(
                    telegram_list[i]["telegram_title"],
                    results[i][1],
                    results[i][2],
                    results[i][3],
                )
                bili_task_manager.update_telegram_info(
                    telegram_list[i]["season_id"], results[i][1], results[i][5]
                )
                ...
                await asyncio.gather(
                    *[
                        sched_bot.send_msg(message=reported_msg, user_id=user_id)
                ...
                await asyncio.gather(
                    *[
                        sched_bot.send_msg(message=reported_msg, group_id=group_id)
                        for group_id in group_list
                    ]
                )

        elif isinstance(
            results[i], (BiliAPIRetCodeError, BiliStatusCodeError, BiliConnectionError)
        ):
            exception_msg = f'[错误报告]\n检测番剧 <{telegram_list[i]["telegram_title"]}> 更新情况时发生错误\n错误类型: {type(results[i])}\n错误信息: {results[i]}'
            logger.error(f"[{__PLUGIN_NAME}]" + exception_msg)


async def follow_telegram(
    id_prefix: str, telegram_id: int, user_id: str, user_type: int
) -> Tuple[bool, str]:
    """根据用户关注节目，修改节目的文件
    ...
    try:
        ...
        if season_id not in bili_task_manager.telegram_list:
            ...
            bili_task_manager.add_telegram_info(
                season_id, telegram_title, episode, is_finish
            )
            if user_type == 0:
        ...
        if (
            user_type == 0
            and user_id in bili_task_manager.telegram_list[season_id]["user_follower"]
            or user_type == 1
            and user_id in bili_task_manager.telegram_list[season_id]["group_follower"]
        ):
            logger.debug(
                f'{__PLUGIN_NAME}番剧 <{bili_task_manager.telegram_list[season_id]["telegram_title"]}> 已关注'
            )
...
async def unfollow_telegram(
    season_id: str, user_id: str, user_type: int
) -> Tuple[bool, str]:
    """根据用户/群取关节目，修改节目文件
    ...
    try:
        if (
            season_id not in bili_task_manager.telegram_list
            or user_type == 0
            and user_id
            not in bili_task_manager.telegram_list[season_id]["user_follower"]
            or user_type == 1
            and user_id
            not in bili_task_manager.telegram_list[season_id]["group_follower"]
        ):
...
async def follow_telegram_list(
    user_id: int, telegram_id_list: List[str], user_type: int
) -> List[List[str]]:
    """个人用户/群关注番剧
    ...
    results = await asyncio.gather(
        *[
            follow_telegram(
                telegram_id[:2], int(telegram_id[2:]), str(user_id), user_type
            )
            for telegram_id in valid_telegram_id_list
        ],
        return_exceptions=True,
    )
...
async def unfollow_telegram_list(
    user_id: int, season_id_list: List[str], user_type: int
) -> List[List[str]]:
    """个人用户/群取关番剧
