...
async def get_latest_news(
    uid: str, pin_news_id: str, record_timestamp: int
) -> Tuple[str, List[Dict]]:
    """根据uid获取最新的动态
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                url=GET_NEWS_BY_UID_API.format(uid), headers=header
            )
    ...
    if (
        ...
        if top_news_id == pin_news_id:
            ...
            logger.debug(
                f"{__PLUGIN_NAME} <{uid}> 的置顶动态发生变化,由{pin_news_id}变为{top_news_id}"
            )
    ...
    for news_item in news_list[has_pin_news:]:
        ...
        news_item_add_type = (
            additional_module["type"] if additional_module else "ADDITIONAL_TYPE_NONE"
        )
