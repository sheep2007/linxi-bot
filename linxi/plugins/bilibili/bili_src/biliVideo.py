...
__PLUGIN_NAME = "[Linxi-Bilibili~视频]"
...
async def check_up_update() -> None:
    ...
    results = await asyncio.gather(
        *[
            bili_client.get_latest_video(
                uid, bili_task_manager.up_list[uid]["latest_timestamp"]
            )
            for uid in check_up_list
        ],
        ...
        if isinstance(results[i], tuple):
            if results[i][0] is True:
                ...
                for user_id in user_list:
                    await schedBot.send_msg(
                        message=textMsg + MessageSegment.image(results[i][4]),
                        user_id=user_id,
                    )
...
async def follow_up(uid: str, user_id: str, user_type: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if (
            user_type == 0
            and user_id in bili_task_manager.up_list[uid]["user_follower"]
            or user_type == 1
            and user_id in bili_task_manager.up_list[uid]["group_follower"]
        ):
...
async def unfollow_up(uid: str, user_id: str, user_type: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if (
            uid not in bili_task_manager.up_list
            or user_type == 0
            and user_id not in bili_task_manager.up_list[uid]["user_follower"]
            or user_type == 1
            and user_id not in bili_task_manager.up_list[uid]["group_follower"]
        ):
