...
__PLUGIN_NAME = "[Linxi-Bilibili~动态]"
    ...
    for i in range(len(check_dynamic_list)):
        if isinstance(results[i], tuple):
            ...
            if results[i][0] is True:
                # 修改缓存中的置顶动态信息
                logger.debug(
                    f'{__PLUGIN_NAME}原置顶动态为{bili_task_manager.dynamic_list[uid]["pin_id_str"]}'
                )
            ...
            if results[i][1] is True:
                ...
                for dynamic_item in results[i][3]:
                    ...
                    if dynamic_item["major_type"] in (
                        ...
                        # add text
                        dynamic_msg.append(
                            MessageSegment.text(dynamic_item["text"] + "\n")
                        )
                        ...
                        if dynamic_item["tag"] == "MAJOR_ARTICLE":
                            ...
                            dynamic_msg.append(
                                MessageSegment.image(dynamic_item["article"]["cover"])
                            )
                        # 添加附加信息
                        # 附加信息是投票
                            ...
                            dynamic_msg.append(
                                MessageSegment.text(
                                    f'<{u_name}>发布了投票: <{dynamic_item["vote"]["desc"]}>\n'
                                )
                            )
                        # 附加信息是视频
                            ...
                            dynamic_msg.append(
                                MessageSegment.text(
                                    f'标题: {dynamic_item["tgc"]["title"]}\n链接: {dynamic_item["tgc"]["jump_url"]}\n'
                                )
                            )
                            dynamic_msg.append(
                                MessageSegment.image(dynamic_item["tgc"]["cover"])
                            )
                        # 附加信息是预约
                            ...
                            dynamic_msg.append(
                                MessageSegment.text(
                                    f'<{u_name}>发布了预约: <{dynamic_item["reserve"]["title"]}>\n{dynamic_item["reserve"]["desc"]}\n'
                                )
                            )
                        dynamic_msg.append(MessageSegment.text("=============\n"))
...
async def follow_dynamic(uid: str, user_id: str, user_type: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if uid not in bili_task_manager.dynamic_list:
            ...
            logger.debug(
                f"{__PLUGIN_NAME}uid in dynamic_list: {bili_task_manager.dynamic_list[uid]}"
            )

        if uid not in bili_task_manager.dynamic_list:
            u_name, pin_id_str, latest_timestamp = await bili_client.init_dynamic_info(
                uid
            )

            bili_task_manager.add_dynamic_info(
                uid, u_name, pin_id_str, latest_timestamp
            )
            logger.debug(
                f"{__PLUGIN_NAME}after update dynamic_list[uid] = {bili_task_manager.dynamic_list[uid]}"
            )
            ...
            logger.debug(
                f"{__PLUGIN_NAME}after follow,  dynamic_list[uid] = {bili_task_manager.dynamic_list[uid]}"
            )
        ...
        if (
            user_type == 0
            and user_id in bili_task_manager.dynamic_list[uid]["user_follower"]
            or user_type == 1
            and user_id in bili_task_manager.dynamic_list[uid]["group_follower"]
        ):
...
async def unfollow_dynamic(uid: str, user_id: str, user_type: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if (
            uid not in bili_task_manager.dynamic_list
            or user_type == 0
            and user_id not in bili_task_manager.dynamic_list[uid]["user_follower"]
            or user_type == 1
            and user_id not in bili_task_manager.dynamic_list[uid]["group_follower"]
        ):
...
async def follow_dynamic_list(
    user_id: int, uid_list: List[str], user_type: int
) -> List[List[str]]:
    """个人用户/群关注动态主
